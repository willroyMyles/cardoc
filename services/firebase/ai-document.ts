...
export async function uriToInlineDataPart(
  uri: string,
...
/**
 * Same as detectAndExtractDocument but accepts pre-converted InlineDataParts,
 * skipping the URI-to-base64 conversion step.
 */
export async function detectAndExtractDocumentFromParts(
  country: CountryCode,
...
Return ONLY a valid JSON object (no markdown code fences) in this exact structure:
{
  ...
  const result = await model.generateContent([...parts, textPart]);
  const raw = result.response.text().trim();
